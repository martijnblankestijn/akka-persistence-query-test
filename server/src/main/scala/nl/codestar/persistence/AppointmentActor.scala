...
class AppointmentActor extends PersistentActor with ActorLogging {
  ...
  def updateState(event: AppointmentEvent): Unit = {
    ...
    state = event match {
      ...
      case AppointmentMoved(advisorId, roomId, startDateTime, branchId) =>
        state.copy(advisorId = advisorId,
                   roomId = roomId,
                   start = startDateTime,
                   branchId = branchId)
    }
  ...
  def persistReadShardedEvent(event: AppointmentEvent)(
      handler: AppointmentEvent ⇒ Unit): Unit = {
    val taggedEvent: Tagged = tag(event)
    super.persist(taggedEvent) { evt: Tagged =>
      handler(evt.payload.asInstanceOf[AppointmentEvent])
    }
  }
  ...
  def initializing(): Receive = {
    ...
    case command: CreateAppointment =>
      ...
      event.bimap(
        ...
        event =>
          persistReadShardedEvent(event) { evt =>
            updateState(evt)
            replyToSender
        }
      )
  ...
  def created(): Receive = {
    ...
    case ReassignAppointment(id, advisorId) =>
      persistReadShardedEvent(AppointmentReassigned(advisorId))(
        updateState _ andThen sendDone)

    case CancelAppointment(id) =>
      persistReadShardedEvent(AppointmentCancelled())(
        updateState _ andThen sendDone)

    case MoveAppointment(id, branchId, advisor, room, start) =>
      val moved = AppointmentMoved(advisor.toString,
                                   room.map(_.toString).orNull,
                                   Some(start),
                                   branchId)
    ...
    case c: CreateAppointment =>
      ...
      sender ! CommandFailed(
        List(AlreadyExists("Appointment already exists", Seq(c.appointmentId))))
  }
  ...
  def cancelled(): Receive = {
    ...
    case c: Command =>
      sender ! CommandFailed(List(
        CancelledError("Appointment already cancelled", Seq(c.appointmentId))))
  }
  ...
  override def unhandled(message: Any): Unit = {
    message match {
      ...
      case _: RecoveryCompleted =>
        log.debug("Recovery complete for {}", persistenceId)
...
object AppointmentActor {
  ...
  case class Appointment(state: State = Tentative,
                         branchId: UUID,
                         advisorId: UUID,
                         roomId: Option[UUID],
                         start: LocalDateTime,
  ...
  case class CreateAppointment(appointmentId: UUID,
                               branchId: UUID,
                               advisorId: UUID,
                               room: Option[UUID],
                               start: LocalDateTime,
                               duration: FiniteDuration)
      extends Command
  case class ReassignAppointment(appointmentId: UUID, advisorId: UUID)
      extends Command
  case class MoveAppointment(appointmentId: UUID,
                             branchId: UUID,
                             advisorId: UUID,
                             roomId: Option[UUID],
                             start: LocalDateTime)
  ...
  private def validateAndCreateEvent(
      c: CreateAppointment): Validation[AppointmentCreated] = {
    (validateInTheFuture(c.start), validateDuration(c.duration))
      .mapN(
        (start, duration) =>
          AppointmentCreated(c.advisorId,
                             c.room,
                             Some(start),
                             Some(duration),
                             c.branchId.toString))

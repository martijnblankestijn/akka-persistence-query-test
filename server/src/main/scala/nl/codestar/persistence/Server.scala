...
//  val config =  ConfigFactory.load("cluster-application").resolve(defaults())
  ...
  val seedNodes = config.getList("akka.cluster.seed-nodes").asScala.mkString(",")
  val remoteHost = config.getString("akka.remote.netty.tcp.hostname")
  val remotePort = config.getString("akka.remote.netty.tcp.port")
  val journalContactpoints = config.getStringList("cassandra-journal.contact-points")
  val snapshotContactpoints = config.getStringList("cassandra-snapshot-store.contact-points")

...
class CassandraOffsetRepository private (session: Session,
  ...
  def bindSaveOffset(offset: Offset): BoundStatement = {
    offset match {
      ...
      case seq: Sequence =>
        statement.bind(eventProcessorId,
                       tag,
                       null,
                       java.lang.Long.valueOf(seq.value))
      case uuid: TimeBasedUUID =>
        statement.bind(eventProcessorId, tag, uuid.value, null)
    }
  }
}
...
object CassandraOffsetRepository {
  def prepareInsert(session: Session, tableName: String): PreparedStatement =
    session.prepare(
      "INSERT INTO appointmentquery.offsetStore (eventProcessorId, tag, timeUuidOffset, sequenceOffset) VALUES (?, ?, ?, ?)")

  def apply(session: Session,
            insertStatement: PreparedStatement,
            eventProcessorId: String,
            tag: String)(implicit executionContext: ExecutionContext)
    : Future[CassandraOffsetRepository] = {

    def createRepositoryFromOffset(offset: Offset) =
      new CassandraOffsetRepository(session,
                                    insertStatement,
                                    eventProcessorId,
                                    tag,
                                    offset)
    def resultSetToRepository(resultSet: ResultSet) =
      createRepositoryFromOffset(extractOffsetFromResultSet(resultSet))

    val tableName = "appointmentquery.offsetStore"
    session
      .executeAsync(
        s"SELECT timeUuidOffset, sequenceOffset FROM $tableName WHERE eventProcessorId = ? AND tag = ?",
        eventProcessorId,
        tag) // delivers an ResultSetFuture (.google.ListenableFuture)
      .map(resultSetToRepository)
  }

  private def extractOffsetFromResultSet(resultSet: ResultSet): Offset =
    extractOffset(resultSet.all().asScala.headOption)

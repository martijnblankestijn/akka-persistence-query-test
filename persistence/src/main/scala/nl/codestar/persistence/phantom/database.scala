...
class AppointmentsDatabase(val keyspace: KeySpaceDef)
    extends Database[AppointmentsDatabase](keyspace) {

  object appointments extends AppointmentRepository with keyspace.Connector

  object appointmentsByBranchId
      extends AppointmentByBranchIdRepository
      with keyspace.Connector
}
